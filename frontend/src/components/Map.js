...
function Map() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchCities, 10000);
        ...
        {loading ? (
          ...
          <ol className="space-y-2 list-none p-0 overflow-y-auto">
            {topCities.map((city, index) => (
              <li
                key={index}
                className="flex justify-between items-center text-sm font-medium bg-indigo-50 p-2 rounded-md transition hover:bg-indigo-100"
              >
                <span className="flex items-center text-indigo-800">
                  <span
                    className={`w-5 h-5 mr-2 rounded-full text-xs font-extrabold flex items-center justify-center ${
                      index === 0
