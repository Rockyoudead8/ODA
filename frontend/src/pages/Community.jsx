...
// ── Right sidebar content (shared between desktop panel & mobile drawer) ──
function RightPanel({ posts, activeUsers, latestPosts, totalLikes, totalComments, topCities, loading, setShowModal, scrollToPost, onClose }) {
  return (
    <div className="space-y-3">
      ...
      <div className="bg-zinc-800/60 border border-zinc-700/50 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="p-4">
          ...
          <p className="text-xs text-zinc-500 mt-1 leading-relaxed">Share city experiences with fellow explorers worldwide.</p>
          ...
          <button
            onClick={() => { setShowModal(true); onClose?.(); }}
            className="w-full mt-3 bg-violet-700 hover:bg-violet-600 text-white text-xs font-semibold py-2 rounded-xl transition active:scale-95"
          >
            + Create Post
          </button>
        </div>
        ...
        <div className="bg-zinc-800/60 border border-zinc-700/50 rounded-2xl p-4 shadow-sm">
          ...
          <div className="space-y-3 divide-y divide-zinc-700/40">
            {latestPosts.slice(0, 4).map((post, i) => (
              <div key={post._id} onClick={() => { scrollToPost(post._id); onClose?.(); }} className="pt-3 first:pt-0 cursor-pointer group">
...
function LeftPanel({ activeTab, setActiveTab, activeUsers, topCities, loading, setShowModal, onClose }) {
  return (
    <>
...
function Community() {
  ...
  // Sidebar open states — separate drawers for left & right
  ...
  const leftProps  = { activeTab, setActiveTab, activeUsers, topCities, loading, setShowModal };
  const rightProps = { posts, activeUsers, latestPosts, totalLikes, totalComments, topCities, loading, setShowModal, scrollToPost };

  return (
    <div className="bg-zinc-900" style={{ height: "calc(100vh - 57px)", overflow: "hidden", display: "flex", flexDirection: "column" }}>

      {/* ── Mobile top bar (hidden on lg+) ── */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-zinc-900 border-b border-zinc-800 lg:hidden shrink-0">
        <button
          onClick={() => { setLeftOpen(true); setRightOpen(false); }}
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition"
        >
          <Menu size={16} /> Menu
        </button>
        <button
          onClick={() => setShowModal(true)}
          className="text-xs font-semibold bg-violet-700 hover:bg-violet-600 text-white px-3 py-1.5 rounded-lg transition"
        >
          + Post
        </button>
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition"
        >
          Info <BarChart2 size={16} />
        </button>
      </div>

      {/* ── Left drawer overlay (mobile / tablet) ── */}
      {leftOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          <div className="w-72 max-w-[85vw] bg-zinc-900 border-r border-zinc-800 flex flex-col overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
            <div className="flex items-center justify-between px-4 pt-4 pb-2 shrink-0">
              <span className="text-sm font-bold text-zinc-100">Menu</span>
              <button onClick={() => setLeftOpen(false)} className="p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-400"><X size={16} /></button>
            </div>
            <LeftPanel {...leftProps} onClose={() => setLeftOpen(false)} />
          </div>
          <div className="flex-1 bg-black/50" onClick={() => setLeftOpen(false)} />
        </div>
      )}

      {/* ── Right drawer overlay (mobile / tablet) ── */}
      {rightOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex justify-end">
          <div className="flex-1 bg-black/50" onClick={() => setRightOpen(false)} />
          <div className="w-80 max-w-[90vw] bg-zinc-900 border-l border-zinc-800 overflow-y-auto p-4" style={{ scrollbarWidth: "thin" }}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-bold text-zinc-100">Info</span>
              <button onClick={() => setRightOpen(false)} className="p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-400"><X size={16} /></button>
            </div>
            <RightPanel {...rightProps} onClose={() => setRightOpen(false)} />
          </div>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="hidden lg:flex w-64 xl:w-72 shrink-0 bg-zinc-900 border-r border-zinc-800 flex-col overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "#52525b #18181b" }}>
          <LeftPanel {...leftProps} />
        </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between mb-3 gap-2">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-semibold text-zinc-100 leading-tight truncate">{post.user?.name ?? "Unknown"}</p>
                            <p className="text-xs text-zinc-500">{timeAgo(post.createdAt)}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="inline-flex items-center gap-1 bg-orange-900/30 border border-orange-800/40 text-orange-400 text-xs font-semibold px-2 sm:px-2.5 py-1 rounded-full">
                            <MapPin size={10} className="shrink-0 hidden sm:inline" />{post.city}
                          </span>
                          {isOwner && (
                      ...
                      {/* Action bar */}
                      <div className="flex items-center gap-0.5 sm:gap-1 mt-3 pt-3 border-t border-zinc-700/40 flex-wrap">
                        ...
                        >
                          <ThumbsUp size={13} fill={isLiked ? "currentColor" : "none"} />{post.likes.length}
                        </button>
                        <span className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 text-xs font-medium text-zinc-500">
                          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                          {post.comments.length}
                        </span>
                        <button onClick={() => sharePost(post)} className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-semibold text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200 transition-colors">
                          <Share2 size={13} /><span className="hidden sm:inline">Share</span>
                        </button>
                        <button onClick={() => toggleSave(post._id)}
                          className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ml-auto ${
                            isSaved ? "text-amber-400" : "text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200"
                          }`}
                        >
                          <Bookmark size={13} fill={isSaved ? "currentColor" : "none"} /><span className="hidden sm:inline">{isSaved ? "Saved" : "Save"}</span>
                        </button>
                      </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-zinc-700/40">
                        <div className="flex gap-2 mb-3">
                          ...
                          />
                          <button onClick={() => addComment(post._id)} className="bg-violet-700 text-white px-2.5 sm:px-3 rounded-xl text-xs font-semibold hover:bg-violet-600 transition active:scale-95">
                            Post
                          </button>
                        </div>
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  {showReplyBox[comment._id] && (
                                    <div className="ml-2 mt-2 flex gap-2">
                                      ...
                                      />
                                      <button onClick={() => addReply(post._id, comment._id)} className="bg-zinc-700 hover:bg-zinc-600 text-zinc-200 px-3 rounded-lg text-xs font-semibold transition">
                                        Send
                                      </button>
                                    </div>
                                  )}
        ...
        <div className="hidden xl:block w-72 shrink-0 overflow-y-auto p-3 space-y-3" style={{ scrollbarWidth: "none" }}>
