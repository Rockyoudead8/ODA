  ...
  if (photoUrl) {
    return (
      <img
        src={photoUrl}
        alt={name || "avatar"}
        className={`${sz} rounded-full object-cover shrink-0 border border-zinc-600/50`}
...
export default function ChatPage({ user }) {
  ...
  const leaveGroup = async () => {
    ...
    try {
      ...
      if (res.ok) { setGroups(prev => prev.filter(g => g._id !== selectedGroup._id)); setSelectedGroup(null); setMessages([]); }
  ...
  if (!user || !user._id) return <div className="p-5 text-zinc-500 text-sm bg-zinc-900 min-h-screen">Loading…</div>;
  ...
  const headerStatusColor = isGroupChat ? "text-violet-400"
    : isTyping ? "text-blue-400" : activeIsOnline ? "text-emerald-400" : "text-zinc-500";

  return (
    <div className="h-screen overflow-hidden flex bg-zinc-900">

      {/* ══ LEFT SIDEBAR ═══════════════════════════════════════════════════════ */}
      <div className="w-72 h-full bg-zinc-900 border-r border-zinc-800 flex flex-col shadow-xl">
              ...
              <div className="absolute top-full left-0 right-0 mt-1.5 bg-zinc-800 border border-zinc-700 rounded-xl max-h-52 overflow-y-auto shadow-2xl z-50">
                {searchResults.length > 0 ? searchResults.map(u => (
                  <div
                    ...
                    onClick={() => {
                      loadMessages(u);
                      setPartners(prev => prev.find(p => p._id === u._id) ? prev : [u, ...prev]);
        ...
        <div className="flex-1 overflow-y-auto py-2" style={{ scrollbarWidth: "thin", scrollbarColor: "#3f3f46 transparent" }}>
          ...
          {partners.length === 0 && groups.length === 0 && (
            <div className="px-4 py-10 text-center">
              ...
              <button onClick={() => setShowModal(true)} className="mt-1.5 text-xs text-violet-400 hover:text-violet-300 font-medium transition">
                Start one
              </button>
            </div>
          )}
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 px-3 py-2.5 mx-2 rounded-xl cursor-pointer transition-all ${
                  isActive ? "bg-violet-900/40 border border-violet-800/50" : "hover:bg-zinc-800"
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className={`block text-sm font-semibold truncate ${isActive ? "text-violet-300" : "text-zinc-200"}`}>
                    {p?.name}
                  </span>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-zinc-900">
        {(selectedUser || selectedGroup) ? (
          <>
            {/* Chat Header */}
            <div className="h-16 px-5 bg-zinc-800/80 border-b border-zinc-700/60 flex items-center gap-3 shrink-0 backdrop-blur-sm">
              <div className="relative">
                <Avatar
                  name={headerName}
                  photoUrl={!isGroupChat ? selectedUser?.profilePhoto : null}
                  size="md"
                  gradient={isGroupChat ? "from-violet-600 to-pink-600" : "from-violet-600 to-indigo-600"}
                />
                {!isGroupChat && activeIsOnline && (
                  <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-400 border-2 border-zinc-800 rounded-full" />
                )}
              </div>
              <div className="flex-1">
                <p className="text-sm font-bold text-zinc-100 leading-tight">{headerName}</p>
            ...
            <div
              ...
              className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-1.5 bg-zinc-900"
              ...
              {messagesLoading && (
                <div className="flex flex-col gap-4 w-full animate-pulse">
                  <div className="flex items-end gap-2 justify-start">
                    ...
                    <div className="h-9 w-36 rounded-2xl rounded-bl-sm bg-zinc-800" />
                  </div>
                  <div className="flex items-end gap-2 justify-end">
                    <div className="h-9 w-52 rounded-2xl rounded-br-sm bg-violet-900/50" />
                  </div>
                  <div className="flex items-end gap-2 justify-start">
                    <div className="w-8 h-8 rounded-full bg-zinc-700 shrink-0" />
                    <div className="flex flex-col gap-1.5 items-start">
                      <div className="h-9 w-60 rounded-2xl rounded-bl-sm bg-zinc-800" />
                      <div className="h-9 w-40 rounded-2xl rounded-bl-sm bg-zinc-800" />
                    </div>
                  </div>
                  <div className="flex items-end gap-2 justify-end">
                    <div className="h-9 w-28 rounded-2xl rounded-br-sm bg-violet-900/50" />
                  </div>
                  <div className="flex items-end gap-2 justify-start">
                    <div className="w-8 h-8 rounded-full bg-zinc-700 shrink-0" />
                    <div className="h-9 w-44 rounded-2xl rounded-bl-sm bg-zinc-800" />
                  </div>
                </div>
              )}
              ...
              {!messagesLoading && messages.map((m, i) => {
                ...
                return (
                  <div key={i} className={`flex ${isMe ? "justify-end" : "justify-start"} items-end gap-2`}>
                    {!isMe && isGroupChat && (
                      <Avatar name={senderName || "?"} photoUrl={senderPhoto} size="sm" gradient="from-zinc-600 to-zinc-500" />
                    )}
                    <div className={`flex flex-col max-w-[60%] ${isMe ? "items-end" : "items-start"}`}>
                      {isGroupChat && !isMe && senderName && (
                        <span className="text-xs text-zinc-500 font-medium mb-1 ml-1">{senderName}</span>
                      )}
                      {m?.image && (
                        <img
                          src={m.image}
                          alt="attachment"
                          className={`max-w-xs rounded-2xl mb-1 shadow-sm object-cover border border-zinc-700/50 ${isMe ? "rounded-br-sm" : "rounded-bl-sm"}`}
                          style={{ maxHeight: 260 }}
                        />
                      )}
                      {m?.text && (
                        <div className={`px-4 py-2.5 text-sm leading-relaxed shadow-sm ${
                          isMe
                            ...
                            : "bg-zinc-800 text-zinc-200 border border-zinc-700/60 rounded-2xl rounded-bl-sm"
                        }`}>
                          {m.text}
                        </div>
                      )}
            ...
            {/* Input area */}
            <div className="px-5 py-3 bg-zinc-800/80 border-t border-zinc-700/60 shrink-0 backdrop-blur-sm">
                ...
                <div className="mb-2 flex items-start gap-2">
                  <div className="relative inline-block">
                    <img
                      src={imagePreview}
                      alt="preview"
                      className="h-20 w-20 object-cover rounded-xl border border-zinc-600 shadow-sm"
                    />
                    <button
                      onClick={clearImage}
                      className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center hover:bg-red-500 transition"
                    >
                      <X size={10} />
                    </button>
                  </div>
              ...
              <div className="flex items-center gap-3">
                {/* Image pick button */}
                <button
                  onClick={() => !imageSending && fileInputRef.current?.click()}
                  title="Attach image"
                  disabled={imageSending}
                  className={`w-9 h-9 flex items-center justify-center rounded-full transition-all shrink-0 ${
                    imageSending
                      ? "text-zinc-600 cursor-not-allowed"
                      : "text-zinc-400 hover:text-violet-400 hover:bg-violet-900/30"
                  }`}
                >
                ...
                <button
                  onClick={sendMessage}
                  disabled={(!text.trim() && !imagePreview) || imageSending}
          ...
          <div className="m-auto text-center px-6">
            ...
            <button
              ...
              className="mt-4 px-5 py-2 bg-violet-700 text-white text-sm font-semibold rounded-full hover:bg-violet-600 transition"
            >
              New Chat
            </button>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-zinc-800 border border-zinc-700 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-700">
              ...
              <button
                onClick={closeModal}
                className="w-7 h-7 flex items-center justify-center rounded-full text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200 transition"
              >
                <X size={14} />
              </button>
            </div>
            ...
            <div
              ...
              style={{ maxHeight: "13rem", scrollbarWidth: "thin", overscrollBehavior: "contain" }}
            >
              {modalUsers.length === 0 ? (
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all ${
                      isSelected ? "bg-violet-900/40" : "hover:bg-zinc-700"
                    }`}
                  >
                    <Avatar name={u.name} photoUrl={u.profilePhoto} size="sm" />
                    <span className={`flex-1 text-sm font-medium ${isSelected ? "text-violet-300" : "text-zinc-200"}`}>
                      {u.name}
                    </span>
            ...
            <div className="flex gap-2 px-5 py-4 border-t border-zinc-700">
              <button
                onClick={closeModal}
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-zinc-300 bg-zinc-700 hover:bg-zinc-600 transition"
              >
                Cancel
              </button>
              <button
